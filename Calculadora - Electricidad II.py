...
class CALCULADORA_ELECTRICIDAD_II:
    ...
    def __init__(self, ventana):
        ...
        # Menú principal
        self.menu_label = ttk.Label(ventana, text="Seleccione una opción:")
        ...
        self.opciones_combobox.pack(pady=5)

        self.boton_seleccionar = ttk.Button(ventana, text="Seleccionar", command=self.seleccionar_opcion)
        self.boton_seleccionar.pack(pady=10)

        # Tipo de filtro (inicialmente oculto)
        self.filtro_label = ttk.Label(self.ventana, text="Seleccione un tipo de filtro:")
        self.filtro_variable = tk.StringVar()
        tipos_filtro = ["Pasa Bajos", "Pasa Altos", "Pasa Banda", "Rechaza Banda"]
        ...
        # Reducción de circuitos (inicialmente oculto)
        self.reducir_label = ttk.Label(self.ventana, text="Seleccione el tipo de reducción:")
        self.reducir_variable = tk.StringVar()
        tipos_reduccion = ["En Serie", "En Paralelo"]
        ...
        # Parámetros del filtro (inicialmente ocultos)
        self.resistencia_label = ttk.Label(self.ventana, text="Resistencia (ohms):")
        ...
        self.boton_respuesta = ttk.Button(ventana, text="Mostrar Respuesta", command=self.respuesta)
        self.boton_respuesta.pack(pady=10)

        # Botón para borrar los datos
        self.boton_borrar = ttk.Button(ventana, text="Borrar", command=self.borrar_entradas)
        ...
        # Parámetros del circuito para reducción en serie o paralelo (inicialmente ocultos)
        self.resistencia1_label = ttk.Label(self.ventana, text="Resistencia 1 (ohms):")
        self.resistencia1_label.pack(pady=5)
        self.resistencia1_entrada = ttk.Entry(self.ventana)
        self.resistencia1_entrada.pack(pady=5)

        self.resistencia2_label = ttk.Label(self.ventana, text="Resistencia 2 (ohms):")
    ...
    def mostrar_calculadora_filtros(self):
        # Ocultar elementos de la reducción de circuitos
        self.reducir_label.pack_forget()
        self.reducir_combobox.pack_forget()
        self.resistencia1_label.pack_forget()
        self.resistencia1_entrada.pack_forget()
        ...
        # Mostrar elementos de la calculadora de filtros
        self.filtro_label.pack(pady=5)
        self.filtro_combobox.pack(pady=5)
        self.resistencia_label.pack(pady=5)
        self.resistencia_entrada.pack(pady=5)
    ...
    def mostrar_reduccion(self):
        # Ocultar elementos de la calculadora de filtros
        self.filtro_label.pack_forget()
        self.filtro_combobox.pack_forget()
        self.resistencia_label.pack_forget()
        self.resistencia_entrada.pack_forget()
        ...
        # Mostrar elementos de la reducción de circuitos
        self.reducir_label.pack(pady=5)
        self.reducir_combobox.pack(pady=5)
        self.resistencia1_label.pack(pady=5)
        self.resistencia1_entrada.pack(pady=5)
    ...
    def calcular_respuesta_filtro(self):
        ...
        # Crear un rango de frecuencias para la respuesta del filtro
        frecuencias = np.logspace(1, 6, 400)
        omega = 2 * np.pi * frecuencias

        # Calcular la respuesta del filtro para la frecuencia especificada
        if filtro_seleccionado == "Pasa Bajos":
            if capacitor_valor != 0:
                ...
                return
        elif filtro_seleccionado == "Pasa Altos":
            if capacitor_valor != 0:
    ...
    def calcular_respuesta_reduccion(self):
        try:
            ...
            self.resultados_label.config(
                text="Ingrese valores numéricos para resistencias, inductores, capacitores y frecuencia.")
            return
        ...
        elif tipo_reduccion == "En Paralelo":
        ...
        texto_resultado += f"Diagrama Fasorial en Serie: {np.abs(impedancia_total_serie):.4f} ohms ∠ {np.angle(impedancia_total_serie, deg=True):.4f} grados\n\n"
        texto_resultado += f"Impedancia Total en Paralelo: {impedancia_total_paralelo}\n"
        texto_resultado += f"Diagrama Fasorial en Paralelo: {np.abs(impedancia_total_paralelo):.4f} ohms ∠ {np.angle(impedancia_total_paralelo, deg=True):.4f} grados\n\n"
